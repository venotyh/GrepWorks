...
export async function renderResults(results, cfg) {
  const now = new Date();
  const ts = now.toISOString().replace('T', 'T').slice(0, 19).replace(/:/g, '-');
  const header = now.toISOString().replace('T', ' ').slice(0, 19);
  const dir = cfg?.output?.dir ?? './evaluation_results';
  const formats = cfg?.output?.format ?? ['md', 'json'];

  if (formats.includes('json')) {
    const p = join(dir, `evaluation-${ts}.json`);
    await writeFile(p, JSON.stringify(results, null, 2), 'utf8');
    console.log(`[render] ${p}`);
  }

  if (formats.includes('md')) {
    const p = join(dir, `evaluation-${ts}.md`);
    await writeFile(p, buildMarkdown(results, header), 'utf8');
    console.log(`[render] ${p}`);
  }
}

function buildMarkdown(results, header) {
